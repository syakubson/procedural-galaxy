...
export function generateDeadSpace() {
  return makeSpecialSystem({
    ...
    ageGyr: 4.6,
    description:
      'Колония молчит уже двенадцать лет, но что-то внутри по-прежнему дышит. Шахтёры подняли из недр древний красный обелиск — и мёртвые перестали быть мёртвыми. То, что заморожено на соседней луне, спало миллиарды лет; теперь оно знает, что его разбудили.',
    history:
      'Здесь рвали расколотую планету ради руды, пока из глубин не подняли Знак. Сначала колония оглохла от шёпота, потом спасательный корабль застрял на орбите как плавучий саркофаг.',
